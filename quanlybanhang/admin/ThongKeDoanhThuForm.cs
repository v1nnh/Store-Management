...
namespace quản_lý_bán_hàng
{
    public partial class ThongKeDoanhThuForm : Form
    {
        ...
        private void ThongKeDoanhThuForm_Load(object sender, EventArgs e)
        {
            ...
            //chart1.Series["ChartAVG"].Points.AddXY("thang 2", 1500);
           // chart1.Series["ChartAVG"].Points.AddXY("thang 3", 1400);
          // chart1.Series["ChartAVG"].Points.AddXY("thang 4", 1200);
            int RowCount = dataGridViewDoanhThu.Rows.Count; //hang`
            int ColumnCount = dataGridViewDoanhThu.Columns.Count; //cot
            int r;
            string ten_tam;
            int giatri;
            for (r = 0; r < RowCount; r++) //chay hang`
            {
                ten_tam = dataGridViewDoanhThu.Rows[r].Cells[0].Value.ToString();
                for (int c = 1; c < ColumnCount; c++)
               {
                   giatri = Convert.ToInt32(dataGridViewDoanhThu.Rows[r].Cells[c].Value);
